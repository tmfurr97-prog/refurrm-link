...
export interface CodeHealthAudit {
  report: string;
  hotspots: { path: string; isHot: boolean }[];
}

export async function analyzeRepoCodebase(
  repoName: string, 
  fileTree: RepoFileTree[]
): Promise<CodeHealthAudit> {
  const ai = getAiClient();
  const limitedTree = fileTree.slice(0, 400).map(f => f.path).join('\n');
  
  const prompt = `You are a Senior Full-Stack Engineer and Security Researcher. 
  
  Perform a deep-dive "Code Health Audit" for the GitHub repository: "${repoName}".
  
  Repository File Structure:
  ${limitedTree}
  
  Provide a professional, actionable report using EXACTLY these section headers in Markdown:
  
  # SECURITY: Identify likely bugs or security vulnerabilities based on the file structure and common patterns in similar tech stacks (e.g., missing .env handling, exposed secrets, dangerous dependencies).
  # PERFORMANCE: Suggest bottlenecks (e.g., lack of caching, inefficient component structure, large bundle size potential).
  # ARCHITECTURE: Evaluation of the project organization against best practices (e.g., SOLID principles, separation of concerns).
  # ROADMAP: A prioritized list of the first 3 things a starting developer should do to take this to the next level.
  
  # HOTSPOTS:
  List the exact file paths from the repository that are most critical or problematic, one per line, prefixed with "- ". (e.g., - src/auth.ts)

  Be specific, technical, and honest. If the project looks great, acknowledge it, but find at least one high-level optimization.`;
  ...
  try {
    ...
    const report = response.text || "Analysis could not be completed at this time.";
    
    // Extract hotspots
    const hotspots: { path: string; isHot: boolean }[] = [];
    const lines = report.split('\n');
    let inHotspotsSection = false;
    lines.forEach(line => {
      if (line.toUpperCase().includes('# HOTSPOTS')) {
        inHotspotsSection = true;
      } else if (inHotspotsSection && line.startsWith('# ')) {
        inHotspotsSection = false;
      } else if (inHotspotsSection && line.trim().startsWith('- ')) {
        const path = line.replace('- ', '').trim();
        hotspots.push({ path, isHot: true });
      }
    });
...
export async function generateRemasteredFiles(
  repoName: string,
  fileTree: RepoFileTree[],
  auditReport: string
  ...
  const prompt = `You are a World-Class Software Architect. 
  
  Based on the following Code Audit Report for the repository "${repoName}":
  ...
  Identify the 3-5 most CRITICAL files that need to be rewritten for better performance, security, or "deployability". 
  
  Output the REMASTERED code for these files in the following JSON format ONLY:
  [
    { "fileName": "path/to/file1.ts", "content": "...optimized code..." },
    { "fileName": "path/to/file2.tsx", "content": "...optimized code..." }
  ]
  
  DO NOT include any explanation or extra text outside the JSON array. Ensure the code is clean, production-ready, and follows modern best practices. Use realistic imports even if you don't have the full content of those files. Focus on "Skeleton" versions that show the correct architectural pattern.`;
  ...
  try {
    ...
    console.error("Gemini Remastering failed:", error);
    return [];
  }
}
